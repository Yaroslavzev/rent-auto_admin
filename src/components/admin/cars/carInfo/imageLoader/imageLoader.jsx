...
export const getTheImages = (setState, id, img, getTheName, sendFile) => {
  return new Promise((resolve, reject) => {
    let images = [];
    axios
      .get(`https://srv.rentauto.xyz/images/models/${id}`, getHeaders())
      .then(res => {
        ...
        if (img === null) {
          const selectedFile = `https://srv.rentauto.xyz/images/models/${id}/${
            listOfImages[0].name
          }`;
...
export const getTheName = (id, listOfImages, img, setState) => {
  ...
  listOfImages.map(item => {
    if (
      `https://srv.rentauto.xyz/images/models/${id}/${item.name}` === img
    ) {
